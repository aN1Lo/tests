...
if logging_off(wd) is False:
    ...
    if is_there(wd, 'ru.gigme.android.app:id/reports_grid'):
        wd.find_element_by_id('ru.gigme.android.app:id/popViewsCount').click()
    time.sleep(1)
    if is_there_by_xpath(wd, '//android.widget.TextView[@text=\'ЛЕНТА\']'):
        if is_there(wd, 'android:id/list'):
            if is_there(wd, 'ru.gigme.android.app:id/rating_photos_gallery'):
                if is_there_by_xpath(wd, '//android.widget.TextView[@text=\'ИЗБРАННОЕ\']'):
                    ...
                    if is_there(wd, 'ru.gigme.android.app:id/friend_photos'):
                        if is_there(wd, 'ru.gigme.android.app:id/empty_view'):
                            if is_there_by_xpath(wd, '//android.widget.TextView[@text=\'РЕАКЦИИ\']'):
